...
namespace NegativeScreen
{
	...
	class OverlayManager : Form
	{
		...
		public const int CYCLE_HOTKEY_ID = 47;
		...
		/// </summary>
		private bool mainLoopPaused = true;
        ...
        {
            currentMonitorIndex = (currentMonitorIndex + 1) % monitorStates.Length;
            string currentMonitorState = monitorStates[currentMonitorIndex];

            foreach (ToolStripMenuItem menuItem in this.contextMenu.Items)
            {
                if (currentMonitorState == "BOTH")
                {
                    menuItem.Checked = true;
                }
                else
                {
                    menuItem.Checked = menuItem.Text.Contains(currentMonitorState);
                }
            }
        ...
        private readonly string[] monitorStates = { "DISPLAY1", "DISPLAY2", "BOTH" };
        ...
        public OverlayManager()
        {
            ...
            if (!NativeMethods.RegisterHotKey(this.Handle, CYCLE_HOTKEY_ID, KeyModifiers.MOD_WIN | KeyModifiers.MOD_ALT, Keys.C))
            {
                throw new Exception("RegisterHotKey(win+alt+C)", Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error()));
            }
		...
		private void UnregisterHotKeys()
		{
            ...
            NativeMethods.UnregisterHotKey(this.Handle, CYCLE_HOTKEY_ID);
				...
				case (int)WindowMessage.WM_HOTKEY:
					switch ((int)m.WParam)
					{
						...
						case DECREASE_TIMER_HOTKEY_ID:
							...
							break;
                        case CYCLE_HOTKEY_ID:
                            CycleMonitors();
                            break;
                        // Additional hotkey event handling...
